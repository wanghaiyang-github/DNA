            ...
            <div class="col-lg-12">
                <section class="panel">
                    ...
                    <div class="panel-body">
                        <form id="queryForm" action="<%=path%>/center/sy/list.html" class="form-horizontal tasi-form"
                              method="post">
            ...
            <div class="col-lg-12">
                <section class="panel">
                    ...
                    <div class="panel-body">
                        <%--<div class="clearfix">
                            <div class="btn-group">
                                    <button id="importSytableBtn" class="btn btn-success green">
                                    导入上样表 <i class="fa fa-plus"></i>
                                </button>
                            </div>
                        </div>
                        ...
                        <table class="table table-striped table-advance table-bordered table-hover">
                            ...
                            <c:forEach items="${caseInfoVOList}" var="caseInfo" varStatus="s">
                                <tr style="font-size: 15px">
                                    ...
                                    <td>
                                        <c:choose>
                                            <c:when test="${caseInfo.syRecordCount gt 0  }">
                                                已上样 ${caseInfo.syRecordCount} 次
                                            </c:when>
                                            <c:when test="${caseInfo.appSyRecordCount gt 0 }">
                                                智慧实验室已上样${caseInfo.appSyRecordCount} 次
                                            </c:when>
                                            <c:otherwise>
                                                未上样
                                            </c:otherwise>
                                    ...
                                    <td>
                                        ...
                                        <input type="hidden" name="syRecordCount" value="${caseInfo.syRecordCount}"/>
                                        <input type="hidden" name="syRecordId" value="${caseInfo.syRecordId}"/>
                                        <input type="hidden" name="taskId" value="${caseInfo.taskId}"/>
                                        <input type="hidden" name="cno" value="${caseInfo.cno}"/>
                                        ...
                                        <c:choose>
                                            <c:when test="${caseInfo.syRecordCount gt 0 }">
                                                ...
                                                <button class="btn btn-info btn-sm" name="downloadSytableBtn"><i
                                                        class="fa fa-file-text"></i> 下载上样表
                                                </button>
                                                <button class="btn btn-info btn-sm" name="downloadBtn"><i
                                                        class="fa fa-download"></i> 下载记录表
                                                </button>
                                            </c:when>
                                            <c:when test="${caseInfo.appSyRecordCount eq 1 }">
                                                <button class="btn btn-info btn-sm" name="appDownloadBtn"><i
                                                        class="fa fa-download"></i> 下载记录表
                                                </button>
                                            </c:when>
                                            <c:when test="${caseInfo.appSyRecordCount gt 1 }">
...
<script>
    $(function () {
        ...
        function delRecord(obj) {
            ...
            $.ajax({
                url: '<%=path%>/center/sy/delByCaseId.html?caseId=' + caseId,
                type: "get",
                dataType: "json",
        ...
        $("button[name='addBtn']", "#syRecordListTable").on('click', function () {
            var caseId = $("input[name='caseId']", $(this).parent()).val();
            location.href = '<%=path%>/center/sy/add.html?caseId=' + caseId;
        });

        $("button[name='viewBtn']", "#syRecordListTable").on("click", function () {
            var caseId = $("input[name='caseId']", $(this).parent()).val();
        ...
        $("button[name='delBtn']", "#syRecordListTable").on("click", function () {
            ...
            var caseId = $("input[name='caseId']", $(this).parent()).val();
            var syRecordCount = $("input[name='syRecordCount']", $(this).parent()).val();
            if (syRecordCount == 1) {
        ...
        $("button[name='downloadSytableBtn']", "#syRecordListTable").on("click", function () {
            var caseId = $("input[name='caseId']", $(this).parent()).val();
            var syRecordCount = $("input[name='syRecordCount']", $(this).parent()).val();
            if (syRecordCount == 1) {
                location.href = '<%=path%>/center/sy/downloadSytable.html?caseId=' + caseId;
            } else {
                location.href = '<%=path%>/center/sy/viewSyRecord.html?caseId=' + caseId;
            }
        });
        ...
        $("button[name='detailsAppDownloadBtn']", "#syRecordListTable").on("click", function () {
            var cno = $("input[name='cno']", $(this).parent()).val();
            var caseId = $("input[name='caseId']", $(this).parent()).val();
            location.href = '<%=path%>/center/sy/viewAppSyRecord.html?cno=' + cno + "&caseId=" + caseId;
        });
        ...
        kkpager.generPageHtml({
            ...
            //链接前部
            hrefFormer: '<%=path%>/center/sy/list',
            //链接尾部
            hrefLatter: '.html',
