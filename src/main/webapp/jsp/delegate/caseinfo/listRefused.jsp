    ...
    <aside>
        <div id="sidebar" class="nav-collapse">
            <ul class="sidebar-menu" id="nav-accordion">
                ...
                <li>
                    <a href="<%=path%>/wt/caseinfo/listRefused.html" class="active">
                        已退案
                    </a>
                </li>
                <li>
                ...
                <div class="col-lg-12">
                    <section class="panel">
                        ...
                        <div class="panel-body">
                            <form id="queryForm" action="<%=path%>/wt/caseinfo/listRefused.html"
                                  class="form-horizontal tasi-form" method="post">

                                <div class="form-group">
                                    <label class="col-sm-2 col-sm-2 control-label text-align-right">案件类型</label>
                                    <div class="col-sm-3">
                                        <select class="form-control" name="entity.caseType" id="caseType"
                                                value="${query.entity.caseType}">
                                            <c:forEach items="${caseTypeList}" var="list" varStatus="s">
                                                <option value="${list.dictCode}"
                                                        <c:if test="${query.entity.caseType eq list.dictCode}">selected</c:if>>${list.dictName}</option>
                                            </c:forEach>
                                        </select>
                                    </div>
                                ...
                                <div class="form-group">
                                    <label class="col-sm-2 col-sm-2 control-label">委托时间</label>
                                    <div class="col-sm-3">
                                        <input class="form_datetime form-control" name="delegateDatetimeStart"
                                               readonly="readonly" type="text" value="<fmt:formatDate value='${query.delegateDatetimeStart}' pattern='yyyy-MM-dd'/>">
                                    </div>
                                    ...
                                    <div class="col-sm-3">
                                        <input class="form_datetime form-control" name="delegateDatetimeEnd"
                                               readonly="readonly" type="text" value="<fmt:formatDate value='${query.delegateDatetimeEnd}' pattern='yyyy-MM-dd'/>">
                                    </div>
                                </div>
                                <div class="form-group">
                            ...
                            <table class="table table-striped table-advance table-bordered table-hover">
                                ...
                                <c:forEach items="${caseInfoList}" var="caseInfo" varStatus="s">
                                    <tr>
                                        ...
                                        <td title="${caseInfo.refuseReason}">
                                            <c:if test="${fn:length(caseInfo.refuseReason) gt 15}">${fn:substring(caseInfo.refuseReason,0,15)} ...</c:if>
                                            <c:if test="${fn:length(caseInfo.refuseReason) lt 16}">${caseInfo.refuseReason}</c:if>
                                        </td>
                                        <td>
...
<script>
    $(function () {
        ...
        $("button[name='supplyBtn']", "#caseListTbody").on("click", function () {
            ...
            location.href = "<%=path%>/wt/reg/1.html?caseId=" + caseId + "&additionalFlag=1";
        });

        $("button[name='editBtn']", "#caseListTbody").on("click", function () {
            var consignmentId = $("input[name='consignmentId']", $(this).parent("td")).val();
            location.href = "<%=path%>/wt/caseinfo/edit.html?consignmentId=" + consignmentId;
        });


        $("button[name='delBtn']", "#caseListTbody").on("click", function () {
            var consignmentId = $("input[name='consignmentId']", $(this).parent("td")).val();
            location.href = "<%=path%>/wt/caseinfo/del.html?consignmentId=" + consignmentId;
        });
        ...
        kkpager.generPageHtml({
            ...
            //链接前部
            hrefFormer: '<%=path%>/wt/caseinfo/listRefused',
            //链接尾部
            hrefLatter: '.html',
