            ...
            <div class="col-lg-12">
                <section class="panel">
                    ...
                    <div class="panel-body">
                        <form id="queryForm" action="<%=path%>/center/pcr/list.html" class="form-horizontal tasi-form"
                              method="post">
                        ...
                        <table class="table table-striped table-advance table-bordered table-hover">
                            ...
                            <c:forEach items="${caseInfoVOList}" var="caseInfo" varStatus="s">
                                <tr>
                                    ...
                                    <td>
                                            <%--<c:if test="${caseInfo.pcrRecordCount eq 0}">
                                                未扩增
                                            </c:if>
                                            <c:if test="${caseInfo.pcrRecordCount gt 0}">
                                                已扩增 ${caseInfo.pcrRecordCount} 次
                                            </c:if>--%>

                                        <c:choose>
                                            <c:when test="${caseInfo.pcrRecordCount gt 0  }">
                                                已扩增 ${caseInfo.pcrRecordCount} 次
                                            </c:when>
                                            <c:when test="${caseInfo.appPcrRecordCount gt 0 }">
                                                智慧实验室已扩增${caseInfo.appPcrRecordCount} 次
                                            </c:when>
                                            <c:otherwise>
                                                未扩增
                                            </c:otherwise>
                                    ...
                                    <td>
                                        ...
                                        <input type="hidden" name="pcrRecordCount" value="${caseInfo.pcrRecordCount}"/>
                                        <input type="hidden" name="pcrRecordId" value="${caseInfo.pcrRecordId}"/>
                                        ...
                                        <c:choose>
                                            ...
                                            <c:when test="${caseInfo.appPcrRecordCount eq 1 }">
                                                <button class="btn btn-info btn-sm" name="appDownloadBtn"><i
                                                        class="fa fa-download"></i> 下载记录表
                                                </button>
                                            </c:when>
                                            <c:when test="${caseInfo.appPcrRecordCount gt 1 }">
...
<script>
    $(function () {
        ...
        function delRecord(obj) {
            ...
            $.ajax({
                url: '<%=path%>/center/pcr/delByCaseId.html?caseId=' + caseId,
                type: "get",
                dataType: "json",
        ...
        $("button[name='addBtn']", "#pcrRecordListTable").on('click', function () {
            var caseId = $("input[name='caseId']", $(this).parent()).val();
            location.href = '<%=path%>/center/pcr/add.html?caseId=' + caseId;
        });

        $("button[name='viewBtn']", "#pcrRecordListTable").on("click", function () {
            var caseId = $("input[name='caseId']", $(this).parent()).val();
        ...
        $("button[name='delBtn']", "#pcrRecordListTable").on("click", function () {
            ...
            var caseId = $("input[name='caseId']", $(this).parent()).val();
            var pcrRecordCount = $("input[name='pcrRecordCount']", $(this).parent()).val();
            if (pcrRecordCount == 1) {
        ...
        $("button[name='detailsAppDownloadBtn']", "#pcrRecordListTable").on("click", function () {
            var cno = $("input[name='cno']", $(this).parent()).val();
            var caseId = $("input[name='caseId']", $(this).parent()).val();
            location.href = '<%=path%>/center/pcr/viewAppPcrRecordDoc.html?cno=' + cno + "&caseId=" + caseId;
        ...
        kkpager.generPageHtml({
            ...
            //链接前部
            hrefFormer: '<%=path%>/center/pcr/list',
            //链接尾部
            hrefLatter: '.html',
