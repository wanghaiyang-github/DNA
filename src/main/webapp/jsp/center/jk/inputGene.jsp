                                    ...
                                    <div class="col-sm-2">
                                        <select class="form-control" id="reagentSelect" name="reagent">
                                            <c:forEach items="${panelInfoList}" var="panelInfo" varStatus="do">
                                                <option value="${panelInfo.panelName}" <c:if test="${panelInfo.panelName eq 'DNATyper 19'}">selected</c:if> >${panelInfo.panelName}</option>
                                            </c:forEach>
                                        </select>
                                    </div>
                                    <label class="col-sm-2 control-label" style="text-align: right;"><strong>CODIS文件</strong></label>
                                    <div class="col-sm-4">
                                        <input type="file" name="codisFile" id="codisFile" multiple="multiple" class="hide"/>
...
<script>
    function viewGeneDetail(obj) {
        ...
        for(var i = 0; i < len; i++){
            ...
            if(gene.geneVal3 != null && gene.geneVal3 != ""){
                htmlStr += ", " + gene.geneVal3;
            }
            if(gene.geneVal4 != null && gene.geneVal4 != ""){
    ...
    $(function() {
        ...
        function importCodisFtn() {
            ...
            $.ajaxFileUpload({
                cache:false,
                url:"<%=path%>/center/jk/uploadCodis.html?reagentName=" + reagentName,
                secureuri:false,
