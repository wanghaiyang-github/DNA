            ...
            <div class="panel-body">
                <form class="form-horizontal tasi-form" action="<%=path%>/center/4/01.html" method="post" id="queryForm">
                    <div class="form-group">
                        ...
                        <label class="col-sm-1 col-sm-1 control-label text-align-right" style="text-align:center;">案件类型</label>
                        <div class="col-sm-3">
                    ...
                    <div class="form-group">
                        <label class="col-sm-1 col-sm-1 control-label">案件性质</label>
                        <div class="col-sm-3">
                        ...
                        <label class="col-sm-1 col-sm-1 control-label" style="text-align:center;">受理时间</label>
                        <div class="col-sm-3">
                    ...
                    <div class="form-group">
                        ...
                        <label class="col-sm-1 col-sm-1 control-label" style="text-align:center;">鉴定书状态</label>
                        <div class="col-sm-3">
                    ...
                    <c:forEach items="${caseInfoList}" var="caseInfo" varStatus="ci">
                        <tr>
                            ...
                            <td>
                                ...
                                <input type="hidden" name="identifyBookStatus" value="${caseInfo.identifyBookStatus}"/>
                                <c:choose>
                                    <c:when test="${caseInfo.identifyBookStatus eq '01'}">
                                        <button class="btn btn-info btn-sm" name="generateBtn"><i class="fa fa-file-o"></i> 生成鉴定书</button>
                                    </c:when>
                                    <c:when test="${caseInfo.identifyBookStatus eq '02'}">
                                        <button class="btn btn-info btn-sm" name="rebuildGenerateBtn"><i class="fa fa-file-o"></i> 重新生成鉴定书</button>
                                        <button name="uploadIdentifyBookBtn" class="btn btn-primary btn-sm"><i class="fa fa-pencil"></i>上传鉴定书</button>
                                        <button class="btn btn-primary btn-sm" name="downloadBtnIssue"><i class="fa fa-cloud-download"></i> 下载鉴定书</button>
                                        <button class="btn btn-success btn-sm" name="signBtn"><i class="fa fa-exchange"></i> 签发</button>
                                    </c:when>
                                    <c:when test="${caseInfo.identifyBookStatus eq '03'}">
                                    ...
                                    <c:when test="${caseInfo.identifyBookStatus eq '04'}">
                                         <button class="btn btn-success btn-sm" name="compareBtn"><i class="fa fa-exchange"></i> 查看&比对</button>
                                    </c:when>
                                </c:choose>
                                <%--<button name="uploadIdentifyBookBtn" class="btn btn-primary btn-sm"><i class="fa fa-pencil"></i>上传鉴定书</button>--%>
                            </td>
...
<div class="row">
    ...
    <div class="modal fade" id="downloadModal" aria-hidden="true" data-backdrop="static"
         data-keyboard="false">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                ...
                <div class="modal-body">
                    <div class="form-group">
                        ...
                        <div class="col-sm-6" style="text-align:center;">
                            <button class="btn btn-primary btn-sm" id="downloadUploadBtn"
                                    name="downloadUploadBtn"><i class="fa fa-cloud-download"></i> 下载上传鉴定书
                            </button>
                        </div>
                        <div class="col-sm-6" style="text-align:center;">
                            <button class="btn btn-primary btn-sm" id="downloadRebuildBtn"
                                    name="downloadRebuildBtn"><i class="fa fa-cloud-download"></i> 下载生成鉴定书
                            </button>
                        </div>
                    </div>
                </div>
    ...
    $(function(){
        ...
        $("button[name='generateBtn']","#notReportTbody").on("click",function(){
            ...
            $.ajax({
                ...
                success:function(data){
                    if (data) {
                        location.href = "<%=path%>/center/4/02.html?entity.id="+caseId;
                    }else {
                        alert("生成失败！");
                    }
                },
        ...
        $("button[name='compareBtn']","#notReportTbody").on("click", function(){
            var caseId = $("input[name='caseId']", $(this).parent()).val();
            var url = "<%=path%>/center/3/compare.html?caseId="
                    +caseId + "&matchLimit=0";
            location.href=url;
        });
       ...
       /* $("#uploadBtn").on("click",function(){
        ...
        $("#downloadUploadBtn").on("click",function(){
            var caseId = $("#caseIdDownloadModel").val();
            $.ajax({
                url:"<%=path%>/center/4/selectIsExist.html?caseId=" + caseId + "&generate=upload",
                type:"get",
                dataType:"json",
                success:function(data){
                    if(data) {
                        location.href = "<%=path%>/center/4/downloadUpload.html?caseId=" + caseId + "&generate=upload";
        ...
        $("#downloadRebuildBtn").on("click",function(){
            var caseId = $("#caseIdDownloadModel").val();
            $.ajax({
                url:"<%=path%>/center/4/selectIsExist.html?caseId=" + caseId + "&generate=rebuild",
                type:"get",
                dataType:"json",
                success:function(data){
                    if(data) {
                        location.href = "<%=path%>/center/4/downloadUpload.html?caseId=" + caseId + "&generate=rebuild";
                    } else {
                        alert("请生成鉴定书！");
                    }
                    $("#downloadModal").modal('hide');
        ...
        $("button[name='rebuildGenerateBtn']","#notReportTbody").on("click",function(){

            var caseId = $("input[name='caseId']",$(this).parent()).val();
            /*var caseNo = $("input[name='caseNo']",$(this).parent()).val();*/
            var identifyBookStatus = $("input[name='identifyBookStatus']",$(this).parent()).val();
            ...
            $.ajax({
                ...
                success:function(data){
                    if (data) {
                        alert("生成成功！");
                        location.href = "<%=path%>/center/4/01.html?caseId="+caseId+"&identifyBookStatus="+identifyBookStatus;
                    }else {
                        alert("生成失败！");
                    }
                },
        ...
        $("button[name='downloadBtnIssue']","#notReportTbody").on("click",function(){
        ...
        $("button[name='signBtn']","#notReportTbody").on("click",function(){
            if (!confirm("确定签发吗?"))
                return;

            var caseId = $("input[name='caseId']",$(this).parent()).val();
            var page = $("#page").val();
            location.href = "<%=path%>/center/4/signIdentifyBook.html?entity.id=" + caseId + "&page=" + page;
        });

        //
        $("button[name='downloadBtn']","#notReportTbody").on("click",function(){
            var caseId = $("input[name='caseId']",$(this).parent()).val();
            location.href="<%=path%>/center/4/downloadUpload.html?caseId="+ caseId + "&generate=download";
        });

        $("button[name='revertBtn']","#notReportTbody").on("click",function(){
        ...
        $("button[name='receiveBtn']","#notReportTbody").on("click",function(){
